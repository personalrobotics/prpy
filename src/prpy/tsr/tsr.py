...
def get_manipulator_no_tilting_tsrchain(manip_eetrans, manip=None):
    """
    ...
    end-effecttor axis that is closest to the world z-axis.
    it.  
    @return string The string that constraints CBiRRT plans to keep 
    end-effector upright.
    """
    T0_ee = manip_eetrans
    if manip is None:
        manipindex = -1
        ...
        manipindex = manip
    # Find which axis of end-effector is aligned with +z of world.
    x_is_up = False;
    y_is_up = False;
    z_is_up = False;
    x_projection = abs(numpy.dot(T0_ee[0:3,0], numpy.array([0,0,1.0])))
    y_projection = abs(numpy.dot(T0_ee[0:3,1], numpy.array([0,0,1.0])))
    z_projection = abs(numpy.dot(T0_ee[0:3,2], numpy.array([0,0,1.0])))
    if x_projection > y_projection:
        if x_projection > z_projection:
            x_is_up = True;
            #logger.debug('World z-axis aligns with end-effector x-axis.')
        else:
            z_is_up = True;
            #logger.debug('World z-axis aligns with end-effector z-axis.')
    else:
        if y_projection > z_projection:
            y_is_up = True;
            #logger.debug('World z-axis aligns with end-effector y-axis.')
        else:
            z_is_up = True;
            #logger.debug('World z-axis aligns with end-effector z-axis.')
    
    constraintTSR = TSR()
    # The w frame is the ee.
    constraintTSR.T0_w = T0_ee;
    constraintTSR.Tw_e = numpy.eye(4);
    if x_is_up:
        constraintTSR.Bw = numpy.array([-100, 100,   -100, 100,   -100, 100,   -numpy.pi,numpy.pi, 0,0, 0,0])
    elif y_is_up:
        constraintTSR.Bw = numpy.array([-100, 100,   -100, 100,   -100, 100,   0,0, -numpy.pi,numpy.pi, 0,0])
    else: # z_is_up
        constraintTSR.Bw = numpy.array([-100, 100,   -100, 100,   -100, 100,   0,0, 0,0, -numpy.pi,numpy.pi])
    constraintTSR.manipindex = manipindex
    constraintTSRChain = TSRChain()
    constraintTSRChain.constrain = True;
    constraintTSRChain.TSRs = [constraintTSR];
